...
@Component({
  ...
  template: `
    <div class="w-full max-w-lg md:max-w-xl p-0 sm:p-6 flex flex-col gap-6 bg-white dark:bg-[#232a3a] rounded-2xl shadow-lg items-center">
      ...
      <h3 class="text-2xl font-bold text-center text-blue-800 dark:text-blue-200">¿Eliminar Asignación?</h3>
      <p class="text-center text-gray-700 dark:text-gray-200">
        ¿Estás seguro de que deseas eliminar la asignación
        <span class="font-semibold text-blue-700 dark:text-blue-300">
          "{{ data.materia }} - {{ data.seccion }} ({{ data.ciclo }})"
...
export class DeleteAsignacionConfirmDialogComponent {
  constructor(
    ...
    @Inject(MAT_DIALOG_DATA) public data: { materia: string; seccion: string; ciclo: string }
  ) {}
